...
async function searchBarEnter(event) {
  ...
  if (event.keyCode == 13) { // Enter key pressed
    if (input === document.activeElement) {
      ...
      return setTimeout(() => loadingDone(), 1000); // delay for smooth loading
    }
...
<h2 class="movie__search--result">"${value}"</h2>`;
  ...
  searchBar.classList.add("movie__search--result-visible"); // show result section
}
...
async function moviesSearch(value) {
  ...
  const array = await searchResults.Search.slice(0, 6); // limit to 6 movies
...
function loadingDone() {
  ...
  targetLoading.classList.remove("movies__list--loading-visible"); // hide spinner
  targetMovie.forEach((movie) => movie.classList.remove("movie__invisible")); // reveal movies
